                ...
                <div class="card bg-body border border-light-subtle shadow-sm rounded-3">
                    <div class="card-header bg-body-tertiary border-bottom border-light-subtle py-3 px-4">
                        <h5 class="fw-bold text-body m-0">
                            <i class="fa-solid fa-building-circle-check me-2 text-primary"></i>Tạo phòng ban mới
                        </h5>
                    </div>
                    <div class="card-body p-4">
                        <form action="{{ route('app.departments.store') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="name" id="name" class="form-control form-control-sm rounded-3 @error('name') is-invalid @enderror" value="{{ old('name') }}" required placeholder="Ví dụ: Phòng Kinh Doanh">
                                @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>

                            <div class="mb-3">
                                <label class="form-label small fw-semibold">Mã nhận diện (Slug) <span class="text-danger">*</span></label>
                                <input type="text" name="slug" id="slug" class="form-control form-control-sm rounded-3 @error('slug') is-invalid @enderror" value="{{ old('slug') }}" required placeholder="Ví dụ: phong-kinh-doanh">
