        ...
        <div
            ...
            <div class="d-flex align-items-center gap-2">
                <button wire:click="previousMonth"
                    ...
                    style="height: 38px; width: 38px;" title="Tháng trước">
                    <i class="fa-solid fa-chevron-left"></i>
                </button>
                <button wire:click="nextMonth"
                    class="btn btn-light btn-sm border border-light-subtle rounded-3 px-3 py-2 fw-semibold d-flex align-items-center justify-content-center"
                    style="height: 38px; width: 38px;" title="Tháng sau">
                    <i class="fa-solid fa-chevron-right"></i>
                </button>
                <button wire:click="goToToday"
                    class="btn btn-light btn-sm border border-light-subtle rounded-3 px-3 py-2 fw-semibold ms-1"
                    style="height: 38px;" title="Hôm nay">
                    Hôm nay
                </button>
                ...
                <div class="form-check form-switch mb-0 ms-2 d-flex align-items-center gap-2">
                    ...
                    <label class="form-check-label text-muted small fw-semibold" for="showGreecoSwitch"
                        style="user-select: none;">Hiện lịch Greeco</label>
                </div>
            </div>
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <button
                    class="btn btn-primary btn-sm px-4 fw-semibold shadow-sm d-flex align-items-center justify-content-center"
                    style="background-color: #2563eb; border-color: #2563eb; border-radius: 8px; height: 38px; cursor: default;">
                    Tháng
                </button>
                <button wire:click="openCreateModal"
                    class="btn btn-outline-primary btn-sm px-3 shadow-sm d-flex align-items-center gap-2 fw-semibold"
        ...
        @forelse($mobileEventDays as $currentDate)
            <section class="ws-agenda-day bg-white shadow-sm">
                <button type="button" class="ws-agenda-day-header"
                    ...
                    <span class="ws-agenda-count">{{ count($this->eventsForDate($calendarData, $currentDate)) }} sự
                        kiện</span>
                </button>
                    ...
                    <div class="d-flex align-items-center gap-2">
                        @if($this->isInsideCurrentMonth($currentDate) && count($eventsForDay) > 0)
                            <span class="text-muted fw-semibold"
                                style="font-size: 0.72rem; color: #64748b;">{{ count($eventsForDay) }}</span>
                        @endif
                        ...
                        @if($this->canAddInCalendarDate($currentDate))
                            <button wire:click.stop="openCreateModal('{{ $this->calendarDayKey($currentDate) }}')"
                                ...
                                title="Thêm sự kiện" style="width: 22px; height: 22px; border-radius: 50%;">
        ...
        <div style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 90%; max-width: 700px; max-height: 80vh; overflow-y: auto; background: var(--ws-modal-bg, #fff); border-radius: 16px; box-shadow: 0 25px 50px rgba(0,0,0,0.15);"
            ...
            <div class="d-flex justify-content-between align-items-center p-4 border-bottom">
                <h5 class="mb-0 fw-bold">
                    <i class="fa-solid fa-calendar-day me-2"></i> Lịch công tác ngày
                    {{ $detailDate ? \Carbon\Carbon::parse($detailDate)->format('d/m/Y') : '' }}
                </h5>
                        ...
                        <div class="mb-3 p-3 rounded-3 border ws-detail-item ws-detail-{{ $evt['color'] }}">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <div>
                                    ...
                                    <span
                                        class="ws-dept-badge badge ms-2 fw-normal fs-85">{{ $evt['department'] ?: 'Nhân viên' }}</span>
                                </div>
        ...
        <div style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 90%; max-width: 550px; background: var(--ws-modal-bg, #fff); border-radius: 16px; box-shadow: 0 25px 50px rgba(0,0,0,0.15);"
            ...
            <form wire:submit.prevent="save" class="p-4">
                ...
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-md-6">
                        <label class="form-label fw-bold text-muted">Ngày kết thúc <span
                                class="fw-normal text-muted">(tùy chọn)</span></label>
                ...
                <div class="row g-3 mb-3">
                    <div class="col-md-6">
                        <label class="form-label fw-bold text-muted">Giờ bắt đầu <span class="fw-normal text-muted">(tùy
                                chọn)</span></label>
                    ...
                    <div class="col-md-6">
                        <label class="form-label fw-bold text-muted">Giờ kết thúc <span
                                class="fw-normal text-muted">(tùy chọn)</span></label>
                ...
                <div class="mb-4">
                    <label class="form-label fw-bold text-muted">Mô tả <span class="fw-normal text-muted">(tùy
                            chọn)</span></label>
                ...
                <div class="mb-3">
                    <div class="form-check form-switch">
                        ...
                        <label class="form-check-label fw-bold text-muted" for="isPrivateSwitch">Sự kiện riêng
                            tư</label>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end gap-2">
                    <button type="button" wire:click="closeFormModal"
                        class="btn btn-light px-4 rounded-8px">Hủy</button>
