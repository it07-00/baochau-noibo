<div>
    <div class="px-3 px-md-4 py-4">
                    ...
                    <div class="d-flex align-items-center flex-grow-1">
                        ...
                        @if (!$loop->last)
                            <div class="flex-grow-1 {{ in_array($key, $completedSteps) ? 'bg-success' : 'bg-secondary-subtle' }}"
                                style="height: 2px; margin-top: -30px; min-width: 20px;"></div>
                        @endif
                    </div>
                @endforeach
            </div>
                    ...
                    <div class="d-flex flex-column align-items-center flex-shrink-0 w-44px">
                        ...
                        @if (!$loop->last)
                            <div class="flex-grow-1 my-1 {{ in_array($key, $completedSteps) ? 'bg-success' : 'bg-secondary-subtle' }}"
                                style="width: 2px; min-height: 20px;"></div>
                        @endif
                    </div>
        ...
        @if ($activeStep)
            <div wire:key="confirm-{{ $activeStep }}" class="card border border-primary shadow-sm mt-3 bg-body">
                <div class="card-header bg-primary-subtle border-primary-subtle py-2 px-3">
                    <span class="fw-bold text-primary-emphasis fs-90">
