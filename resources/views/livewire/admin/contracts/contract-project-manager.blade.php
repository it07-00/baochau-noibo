<div>
    ...
    <div class="page-header d-flex align-items-center justify-content-between mb-4">
        <div>
            <h4 class="mb-0">Kỹ thuật & Ứng phó SC</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('app.dashboard') }}">Bảng thống kê</a></li>
                    <li class="breadcrumb-item active">Kỹ thuật & Ứng phó SC</li>
                </ol>
            </nav>
        </div>
    ...
    @php
        $canBulkDelete =
            auth()->user()->can('contracts-project.delete') &&
            !auth()
                ->user()
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white py-3 d-flex align-items-center justify-content-between border-bottom">
            ...
            <button class="btn btn-sm btn-link text-muted" type="button" data-bs-toggle="collapse"
                data-bs-target="#filterBodyProject">−</button>
        </div>
                    ...
                    @unless (auth()->user()->hasAnyRole(['tu-van', 'ky-thuat']))
                        ...
                        <div class="col-md-3 d-flex align-items-end gap-3 pb-2">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="pj_offset"
                                    wire:model.live="filter.is_offset">
                                <label class="form-check-label " for="pj_offset">Có bù trừ</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="pj_roomfund"
                                    wire:model.live="filter.has_room_fund">
                                <label class="form-check-label " for="pj_roomfund">Có quỹ phòng</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="pj_overdue"
                                    wire:model.live="filter.is_overdue">
                                <label class="form-check-label " for="pj_overdue">Trễ hạn</label>
                            </div>
                        </div>
                ...
                <div class="modal-body p-0">
                    @if ($selectedDoc)
                        ...
                        <ul class="nav nav-tabs px-4 pt-3 bg-white" role="tablist">
                            <li class="nav-item">
                                <button class="nav-link active fw-semibold" data-bs-toggle="tab"
                                    data-bs-target="#tab-info-project-{{ $selectedDoc->id }}" type="button">
                                    <i class="bi bi-info-circle me-1"></i>Thông tin HĐ
                                </button>
                            </li>
                            <li class="nav-item">
                                <button class="nav-link fw-semibold" data-bs-toggle="tab"
                                    data-bs-target="#tab-progress-project-{{ $selectedDoc->id }}" type="button">
                            ...
                            @unless (auth()->user()->hasAnyRole(['tu-van', 'ky-thuat']))
                                <li class="nav-item">
                                    <button class="nav-link fw-semibold" data-bs-toggle="tab"
                                        data-bs-target="#tab-payment-project-{{ $selectedDoc->id }}" type="button">
                        ...
                        <div class="tab-content">
                            {{-- Tab 1: Thông tin HĐ --}}
                            <div class="tab-pane fade show active" id="tab-info-project-{{ $selectedDoc->id }}"
                                role="tabpanel">
                            ...
                            {{-- Tab 2: Tiến độ --}}
                            <div class="tab-pane fade" id="tab-progress-project-{{ $selectedDoc->id }}"
                                role="tabpanel">
                                <livewire:admin.contracts.contract-workflow-progress :contractType="'project'" :contractId="$selectedDoc->id"
                                    :key="'progress-project-' . $selectedDoc->id" />
                            </div>

                            @unless (auth()->user()->hasAnyRole(['tu-van', 'ky-thuat']))
                                {{-- Tab 3: Lịch thanh toán --}}
                                <div class="tab-pane fade" id="tab-payment-project-{{ $selectedDoc->id }}"
                                    role="tabpanel">
                                    <livewire:admin.contracts.contract-payment-schedule-manager :contractType="'project'"
                                        :contractId="$selectedDoc->id" :key="'payment-project-' . $selectedDoc->id" />
                                </div>
                            @endunless
                        </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content overflow-hidden border-0 shadow-lg">
                <div class="modal-header bg-primary py-3">
                    <h5 class="modal-title fw-bold modal-title-custom">{{ $isEditing ? 'Chỉnh sửa' : 'Thêm' }} Hợp
                        Đồng Dự Án</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control" wire:model="formData.info_source"
                                list="info-source-list-project" placeholder="Nhập hoặc chọn nguồn...">
                            <datalist id="info-source-list-project">
                                @foreach ($info_sources as $src)
                                    <option value="{{ $src }}">
                                @endforeach
                            </datalist>
            ...
            <div class="modal-content border-0 shadow-lg">
                ...
                <div class="modal-body p-0">
                    @if ($workflowContractId)
                        <livewire:admin.contracts.contract-workflow-panel :contractType="'project'" :contractId="$workflowContractId"
