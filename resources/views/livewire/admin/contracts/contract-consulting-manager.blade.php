<div>
    ...
    <div class="page-header d-flex align-items-center justify-content-between mb-4">
        <div>
            <h4 class="mb-0">HĐ Pháp lý & Hồ sơ MT</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('app.dashboard') }}">Bảng thống kê</a></li>
                    <li class="breadcrumb-item active">HĐ Pháp lý & Hồ sơ MT</li>
                </ol>
            </nav>
        </div>
            ...
            <div class="modal-content overflow-hidden border-0 shadow-lg">
                ...
                <div class="modal-body p-0">
                    ...
                    <ul class="nav nav-tabs px-4 pt-3 bg-white" role="tablist">
                        <li class="nav-item">
                            <button class="nav-link active fw-semibold" data-bs-toggle="tab" data-bs-target="#tab-info-consulting-{{ $selectedDoc->id }}" type="button">
                                <i class="bi bi-info-circle me-1"></i>Thông tin HĐ
                            </button>
                        </li>
                        <li class="nav-item">
                            <button class="nav-link fw-semibold" data-bs-toggle="tab" data-bs-target="#tab-progress-consulting-{{ $selectedDoc->id }}" type="button">
                                <i class="bi bi-diagram-3 me-1"></i>Tiến độ hoàn thành
                            </button>
                        </li>
                        @unless(auth()->user()->hasAnyRole(['tu-van', 'ky-thuat']))
                        <li class="nav-item">
                            <button class="nav-link fw-semibold" data-bs-toggle="tab" data-bs-target="#tab-payment-consulting-{{ $selectedDoc->id }}" type="button">
                    ...
                    {{-- Tab 1: Thông tin HĐ --}}
                    <div class="tab-pane fade show active" id="tab-info-consulting-{{ $selectedDoc->id }}" role="tabpanel">
                    <table class="table table-bordered mb-0">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><span class="badge bg-{{ $selectedDoc->status_color }}">{{ $selectedDoc->status_label }}</span></td>
                            </tr>
                            <tr>
                    ...
                    {{-- Tab 2: Tiến độ --}}
                    <div class="tab-pane fade" id="tab-progress-consulting-{{ $selectedDoc->id }}" role="tabpanel">
                    ...
                    <div class="tab-pane fade" id="tab-payment-consulting-{{ $selectedDoc->id }}" role="tabpanel">
                        <livewire:admin.contracts.contract-payment-schedule-manager
                            :contractType="'consulting'"
                            :contractId="$selectedDoc->id"
                            :key="'payment-consulting-' . $selectedDoc->id"
                        />
                    </div>
                    @endunless
                    </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content overflow-hidden border-0 shadow-lg">
                <div class="modal-header bg-primary py-3">
                    <h5 class="modal-title fw-bold modal-title-custom">{{ $isEditing ? 'Chỉnh sửa' : 'Thêm' }} Hợp Đồng Tư Vấn</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" wire:model="formData.province" list="province-list-consulting">
                            <datalist id="province-list-consulting">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control" wire:model="formData.info_source" list="info-source-list-consulting" placeholder="Nhập hoặc chọn nguồn...">
                            <datalist id="info-source-list-consulting">
                                @foreach($info_sources as $src)
                                    <option value="{{ $src }}">
                                @endforeach
                            </datalist>
                        ...
                        <div class="col-md-12 d-flex gap-4 pt-1">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="form_ct_offset" wire:model="formData.is_offset">
                                <label class="form-check-label" for="form_ct_offset">Có bù trừ</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="form_ct_roomfund" wire:model="formData.has_room_fund">
                                <label class="form-check-label" for="form_ct_roomfund">Có quỹ phòng</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="form_ct_overdue" wire:model="formData.is_overdue">
                                <label class="form-check-label" for="form_ct_overdue">Trễ hạn</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="form_ct_renewal" wire:model="formData.is_renewal">
                                <label class="form-check-label" for="form_ct_renewal">Tái ký</label>
                            </div>
                        </div>
    ...
    <script>
        ...
        window.addEventListener('openAssignModal', () => {
            new bootstrap.Modal(document.getElementById('assignModalConsulting')).show();
        });
        Livewire.on('closeAssignModal', () => {
            bootstrap.Modal.getInstance(document.getElementById('assignModalConsulting'))?.hide();
        });
