<div>
    ...
    @php
        $canBulkDelete = auth()->user()->can('contracts-sustainability.delete');
    @endphp
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white py-3 d-flex align-items-center justify-content-between border-bottom">
            ...
            <button class="btn btn-sm btn-link text-muted" type="button" data-bs-toggle="collapse"
                data-bs-target="#filterBodySustainability">−</button>
        </div>
                    ...
                    @unless (auth()->user()->hasAnyRole([\App\Enums\Role::TU_VAN->value, \App\Enums\Role::KY_THUAT->value]))
                        ...
                        <div class="col-md-3 d-flex align-items-end flex-wrap gap-3 pb-2">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="sus_offset"
                                    wire:model.live="filter.is_offset">
                                <label class="form-check-label " for="sus_offset">Có bù trừ</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="sus_roomfund"
                                    wire:model.live="filter.has_room_fund">
                                <label class="form-check-label " for="sus_roomfund">Có quỹ phòng</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="sus_overdue"
                                    wire:model.live="filter.is_overdue">
                                <label class="form-check-label " for="sus_overdue">Trễ hạn</label>
                            </div>
                        </div>
    ...
    <div class="card border-0 shadow-sm">
        ...
        <div class="table-responsive" style="min-height:350px;">
            <table class="table table-hover align-middle mb-0 table-xs">
                ...
                <tbody>
                    @forelse($docs as $doc)
                        <tr class="border-bottom border-light" wire:key="sustainability-row-{{ $doc->id }}">
                            @if ($canBulkDelete)
                            ...
                            <td>
                                <span
                                    class="badge bg-success-subtle text-success border border-success-subtle ">{{ $doc->loai_dich_vu ?: '-' }}</span>
                            </td>
                            @unless (auth()->user()->hasAnyRole([\App\Enums\Role::TU_VAN->value, \App\Enums\Role::KY_THUAT->value]))
                            ...
                            <td class="text-center pe-4">
                                <div class="d-flex justify-content-center gap-2">
                                    ...
                                    @can('contracts-sustainability.edit')
                                        ...
                                        @endif
                                    @endcan
                                    @can('contracts-sustainability.delete')
                                        @php
                                            $canDelete =
                                                !auth()->user()->hasRole(\App\Enums\Role::TP_KINH_DOANH->value) ||
                                                $doc->staff_id === auth()->id();
                    ...
                    @if ($selectedDoc)
                        ...
                        <ul class="nav nav-tabs px-4 pt-3 bg-white" role="tablist">
                            <li class="nav-item">
                                <button class="nav-link active fw-semibold" data-bs-toggle="tab"
                                    data-bs-target="#tab-info-sustainability-{{ $selectedDoc->id }}" type="button">
                                    <i class="bi bi-info-circle me-1"></i>Thông tin HĐ
                                </button>
                            </li>
                            <li class="nav-item">
                                <button class="nav-link fw-semibold" data-bs-toggle="tab"
                                    data-bs-target="#tab-progress-sustainability-{{ $selectedDoc->id }}"
                                    type="button">
                            ...
                            @unless (auth()->user()->hasAnyRole([\App\Enums\Role::TU_VAN->value, \App\Enums\Role::KY_THUAT->value]))
                                <li class="nav-item">
                                    <button class="nav-link fw-semibold" data-bs-toggle="tab"
                                        data-bs-target="#tab-payment-sustainability-{{ $selectedDoc->id }}"
                                        type="button">
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane fade show active"
                                id="tab-info-sustainability-{{ $selectedDoc->id }}" role="tabpanel">
                                <table class="table table-bordered mb-0">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><span
                                                    class="badge bg-success text-white">{{ $selectedDoc->loai_dich_vu ?: '-' }}</span>
                                            </td>
                                        </tr>
                                        @unless (auth()->user()->hasAnyRole([\App\Enums\Role::TU_VAN->value, \App\Enums\Role::KY_THUAT->value]))
                                            <tr>
                            ...
                            {{-- Tab 2: Tiến độ --}}
                            <div class="tab-pane fade" id="tab-progress-sustainability-{{ $selectedDoc->id }}"
                                role="tabpanel">
                            ...
                            @unless (auth()->user()->hasAnyRole([\App\Enums\Role::TU_VAN->value, \App\Enums\Role::KY_THUAT->value]))
                                ...
                                <div class="tab-pane fade" id="tab-payment-sustainability-{{ $selectedDoc->id }}"
                                    role="tabpanel">
                                    <livewire:admin.contracts.contract-payment-schedule-manager :contractType="'sustainability'"
                                        :contractId="$selectedDoc->id" :key="'payment-sustainability-' . $selectedDoc->id" />
                                </div>
                            @endunless
                        </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" wire:model="formData.province"
                                list="province-list-sustainability" autocomplete="off">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" wire:model="formData.info_source"
                                list="info-source-list-sustainability" placeholder="Nhập hoặc chọn nguồn...">
                            <datalist id="info-source-list-sustainability">
                                @foreach ($info_sources as $src)
                                    <option value="{{ $src }}">
                                @endforeach
                            </datalist>
    ...
    @push('scripts')
        <script>
            ...
            window.addEventListener('openAssignModal', () => {
                new bootstrap.Modal(document.getElementById('assignModalSustainability')).show();
            });
            Livewire.on('closeAssignModal', () => {
                bootstrap.Modal.getInstance(document.getElementById('assignModalSustainability'))?.hide();
            });
            window.addEventListener('openWorkflowModal', () => {
                new bootstrap.Modal(document.getElementById('workflowModalSustainability')).show();
            });
        </script>
    @endpush
</div>
