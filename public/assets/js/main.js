...
const saerchList = () => {
	...
	// Determine base path dynamically based on main.js src attribute to prevent 404s on subroutes
	const mainScript = document.querySelector('script[src*="main.js"]');
	let basePath = "";
	if (mainScript) {
		const src = mainScript.getAttribute("src");
		const match = src.match(/(assets\/)?js\/main\.js/);
		if (match) {
			basePath = src.substring(0, match.index);
		}
	}

	// JSON load
	$.getJSON(basePath + "assets/ajax/search.json", function(data) {
