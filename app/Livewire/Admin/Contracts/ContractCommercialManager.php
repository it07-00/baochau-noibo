...
class ContractCommercialManager extends Component
{
    ...
    {
        abort_unless(auth()->user()->can(Permission::CONTRACTS_COMMERCIAL_EDIT->value), 403);
    ...
    public function save(): void
    {
        ...
        if (!$user->can($this->isEditing ? Permission::CONTRACTS_COMMERCIAL_EDIT->value : Permission::CONTRACTS_COMMERCIAL_CREATE->value)) {
    ...
    public function updateStatus(int $id, string $status): void
    {
        ...
        abort_unless($user->can(Permission::CONTRACTS_COMMERCIAL_EDIT->value), 403);
    ...
    public function delete(int $id): void
    {
        ...
        abort_unless($user->can(Permission::CONTRACTS_COMMERCIAL_DELETE->value), 403);
    ...
    {
        abort_unless(auth()->user()->can(Permission::CONTRACTS_COMMERCIAL_CREATE->value), 403);
        $doc = ContractResearch::findOrFail($id);
        $this->resetForm();
    ...
    public function saveAssign(): void
    {
        ...
        foreach ($this->assignUserIds as $userId) {
            ...
            if ($user && $user->id !== auth()->id()) {
                $user->notify(new ContractAssignedNotification('commercial', $this->assignContractId, $contractLabel, auth()->user()->name));
            }
        }
    ...
    public function addProgressNote(int $contractId): void
    {
        ...
        foreach ($recipients->unique('id') as $recipient) {
            if ($recipient->id !== auth()->id()) {
                $recipient->notify(new ContractProgressNoteNotification('commercial', $contractId, $contractLabel, \Illuminate\Support\Str::limit($noteText, 50), auth()->user()->name));
            }
        }
    }
    ...
    public function exportExcel(): \Symfony\Component\HttpFoundation\StreamedResponse
    {
        ...
        }, 'HopDong_ThuongMai_' . now()->format('d_m_Y') . '.xls', [
