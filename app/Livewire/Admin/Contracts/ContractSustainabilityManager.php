...
class ContractSustainabilityManager extends Component
{
    ...
    {
        abort_unless(auth()->user()->can(Permission::CONTRACTS_SUSTAINABILITY_EDIT->value), 403);
    ...
    public function save(): void
    {
        ...
        if (!$user->can($this->isEditing ? Permission::CONTRACTS_SUSTAINABILITY_EDIT->value : Permission::CONTRACTS_SUSTAINABILITY_CREATE->value)) {
    ...
    public function updateStatus(int $id, string $status): void
    {
        ...
        abort_unless($user->can(Permission::CONTRACTS_SUSTAINABILITY_EDIT->value), 403);
        ...
        ContractSustainability::findOrFail($id)->update($updateData);
    ...
    public function delete(int $id): void
    {
        ...
        abort_unless($user->can(Permission::CONTRACTS_SUSTAINABILITY_DELETE->value), 403);
    ...
    {
        abort_unless(auth()->user()->can(Permission::CONTRACTS_SUSTAINABILITY_CREATE->value), 403);
        $doc = ContractSustainability::findOrFail($id);
        $this->resetForm();
    ...
    public function bulkDeleteSelected(): void
    {
        ...
        $docs = ContractSustainability::whereIn('id', $selectedIds)->get();
        foreach ($docs as $doc) {
    ...
    {
        $this->selectedDoc = ContractSustainability::with(['customer', 'staff', 'department', 'assignments.user', 'assignments.assigner', 'handler'])->find($id);
        if ($this->selectedDoc) {
    ...
    public function addProgressNote(int $contractId): void
    {
        ...
        $assignmentUserIds = ContractAssignment::where('assignable_type', ContractSustainability::class)
    ...
    public function exportExcel(): \Symfony\Component\HttpFoundation\StreamedResponse
    {
        ...
        $query = ContractSustainability::with(['customer', 'staff', 'department', 'handler'])
    ...
    public function render()
    {
        ...
        $query = ContractSustainability::with(['customer', 'staff', 'department', 'assignments.user', 'handler'])
            ->when($this->search, function ($q) {
                $q->where(function ($sq) {
                    $sq->where('shd_bc', 'like', '%' . $this->search . '%')
                        ->orWhereHas('customer', function ($csq) {
        ...
        return view('livewire.admin.contracts.contract-sustainability-manager', [
            ...
            'provinces'          => ContractSustainability::whereNotNull('province')->where('province', '!=', '')
