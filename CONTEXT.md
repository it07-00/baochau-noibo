...
- Thẻ sự kiện và chi tiết ngày hiển thị màu nền nhạt pastel dịu mắt cùng chữ đậm tương phản cao.
...
Trong form tạo/chỉnh sửa báo giá (`QuotationManager`) cũng như trong tất cả form tạo/chỉnh sửa hợp đồng (`ContractWasteManager`, `ContractConsultingManager`, `AbstractContractGenericManager`), trường "Dịch vụ cung cấp" (trên báo giá) và "Nội dung dịch vụ" (trên hợp đồng) đều sử dụng trait chung `HasMultiServiceSelection` cho phép tích chọn nhiều dịch vụ từ danh sách 11 dịch vụ chuẩn (Quan trắc môi trường lao động, Quan trắc môi trường, Báo cáo công tác bảo vệ môi trường, Đăng ký môi trường, Giấy phép môi trường, Ứng phó sự cố, Phân loại lao động, Kiểm kê khí nhà kính, Giảm phát thải, Thu gom Chất thải, Hủy hàng) và/hoặc tùy chọn "Tự nhập dịch vụ khác". Khi chuyển từ Báo giá sang Hợp đồng (`quotation_id`), danh sách dịch vụ đã chọn trên Báo giá tự động được phân tích và trích xuất nguyên vẹn sang các ô chọn trên Hợp đồng. Dữ liệu được lưu dưới dạng chuỗi ghép nối bằng dấu phẩy trong CSDL (`service` trên báo giá, `service_content` trên hợp đồng) đảm bảo tương thích 100%.
...
- Validation form admin dùng seam tập trung tại `public/assets/js/livewire-validation-feedback.js`: sau mỗi phản hồi Livewire, error bag được ánh xạ theo `wire:model` để gắn trạng thái Bootstrap `is-invalid`, hiển thị `invalid-feedback`, danh sách lỗi, mở modal/tab/collapse liên quan và focus trường lỗi đầu tiên. Form HTTP truyền thống nhận danh sách lỗi chung qua `admin.partials.validation-summary` trong layout. Validation nghiệp vụ và thông báo tiếng Việt vẫn phải khai báo ở component/request; cơ chế giao diện chung không thay thế server-side validation.
